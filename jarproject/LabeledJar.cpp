...
// 2021-03-28: Add showInfo() implementation.
...
LabeledJar::LabeledJar(std::string label, int numUnits)
    : JarType(numUnits), label(label) {
  std::cout << "Getting labeled jar called " << label << " ready with "
            << numUnits << " units" << std::endl;
}
...
void LabeledJar::showInfo() {
  std::cout << label << " jar with " << quantity() << " units" << std::endl;
}
